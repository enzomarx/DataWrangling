        ...
        for page_num in range(num_pages):
            ...
            for line in lines:
                if re.match(r'\d{2}/\d{2}/\d{2}', line):  # Verificando se a linha começa com uma data
                    table.append(line)
                elif table:  # cmd anexar a linha à última linha se não estiver vazia
                    table[-1] += ' ' + line
            tables.extend(table)
    return tables
...
def generate_excel():
    if pdf_path and excel_path:
        try:
            ...
            valid_tables_found = False  # Flag to track/sinalizador de rastreio '' if at least one valid table is found
            with pd.ExcelWriter(excel_path) as writer:  # cuz automatically close the writer
                for i, table in enumerate(tables):
                    try:
                        df = pd.read_csv(io.StringIO(table), sep="\s+")
                        # Skip tables with unexpected column counts / lidar com erros /->>1
                        if len(df.columns) != 10:
                            print(f"Tabela {i+1} possui um número inválido de colunas. Pulando...")
                            continue
                        df.to_excel(writer, sheet_name=f'Sheet{i+1}', index=False)
                        valid_tables_found = True  # Set flag/sinalizador ! True ! if a valid table is found
